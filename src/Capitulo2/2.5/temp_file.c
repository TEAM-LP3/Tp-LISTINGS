...
temp_file_handle write_temp_file(char *buffer, size_t length)
{
    ...
    //printf("Buffer wr %d", sizeof(*buffer));
    ...
    printf("Archivos temporales creados en /tmp\n");
    return fd;
}
...
char *read_temp_file(temp_file_handle temp_file, size_t *length)
{
    ...
    printf("Leyendo Archivos temporales creados en /tmp\nEnter para continuar");
    getchar();
...
int main(void)
{
    ...
    char *mesg = "Prueba archivos temporales (mkstemp)\n";
    size_t *p = malloc(sizeof(mesg));
    *p = 100;
    int n;
    //temp_file_handle tp = write_temp_file(mesg,*p);
    read_temp_file(write_temp_file(mesg, *p), (size_t *)p);
